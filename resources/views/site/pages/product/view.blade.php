                ...
                <div class="row">
                    @foreach($similars as $similar)
                        <div class="col-md-3 text-center">
                            <div class="product-entry">
                                @foreach($similar->images as $key => $image)
                                    @if($key == 0)
                                        <div class="product-img" style="background-image: url('/uploads/images/products/{{$image->url}}');">
                                            <p class="tag"><span class="new">New</span></p>
                                            <div class="cart">
                                                <p>
                                                    <span class="addtocart"><a href="{{route('site.product.show',['id'=> $similar->id,'slug'=>$similar->slug])}}"><i class="icon-shopping-cart"></i></a></span>
                                                    <span><a href="{{route('site.product.show',['id'=> $similar->id,'slug'=>$similar->slug])}}"><i class="icon-eye"></i></a></span>
                                ...
                                @endforeach
                                <div class="desc">
                                    <h3><a href="shop.html">{{$similar->name}}</a></h3>
                                    <p class="price"><span>
                                    @if(!is_null($similar->offerprice))
                                                ${{$similar->offerprice}}
                                            @else
                                                @foreach($similar->prices as $key => $price)
                                                    @if($key == 0)
                                                        {{$price->originalprice}}
                                                    @endif
                                                @endforeach
